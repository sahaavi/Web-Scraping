...
# you might be wondering we already used the driver.maximize_window() method in the previous audible_bot.py example, 
# why do we need to set the window size again?
# the reason is that the maximize_window() method doesn't work well in headless mode, so we need to set the window size manually
...
books_df.to_csv('audible_books_headless.csv', index=False) # save dataframe to csv file
